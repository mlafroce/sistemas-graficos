...
const float sunlightIntensity = 0.5;
...
vec3 directionalLight() {
    ...
    return directionalLightColor * max (dot(vNormal, lightVec), 0.0) * sunlightIntensity;
}
...
vec3 diffuseLight(vec3 torchPos, vec3 torchColor, float distanceFactor) {
    ...
    return torchColor * max (dot(vNormal, normalize(torchLightVec)), 0.1) * torchIntensity * torchIntensity;
}
...
vec3 reflectionLight(vec3 torchPos, vec3 torchColor) {
    ...
    return torchColor * reflectionIntensity / max(torchDistance, 1.0);
}
...
void main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        torchLight += diffuseLight(torchPos, torchLightColor, 0.3);
        torchLight += reflectionLight(torchPos, torchLightColor) / 2.0;
    }
    vec3 sunLight = directionalLight();
   // vec3 light = vec3(ambientLightVec[0], ambientLightVec[1], ambientLightVec[2]) + torchLight + sunLight;
